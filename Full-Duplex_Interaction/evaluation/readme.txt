...
（2）将拒识场景（Rejection）的四个子场景的RESUME分数（注意场景Pause_Handling和Third-party_Speech_before使用了拒识率（rejection_rate），但表达的含义是一致的，取百分号前面的数字）取平均得到总分（最终结果的满分为100，所以取完平均数之后要记得先乘以100，再四舍五入）。
