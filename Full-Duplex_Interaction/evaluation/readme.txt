总指标计算：
（1）将打断（Interruption）场景的五个子场景的RESPOND分数取平均得到总分（满分为100，所以要记得先乘以100）。
（2）将拒识场景（Rejection）的四个子场景的RESUME分数（满分为100，所以要记得先乘以100）（注意场景Pause_Handling和Third-party_Speech_before使用了拒识率（rejection_rate），但表达的含义是一致的，取百分号前面的数字）取平均得到总分（满分为100）。
